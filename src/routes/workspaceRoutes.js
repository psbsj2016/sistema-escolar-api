...
// ============================================================================
// 🚀 TÚNEL DE CONEXÃO EM TEMPO REAL E VIA VERDE (SSE)
// ============================================================================
...
// ============================================================================
// 🖼️ IDENTIDADE VISUAL DO GRUPO E OUTRAS ROTAS GERAIS
// ============================================================================
router.get('/chat/info/:turmaId', verificarToken, async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ error: 'Erro ao buscar informações.' }); }
});
...
router.post('/chat/:turmaId', verificarToken, async (req, res) => {
    try {
        ...
        workspaceStream.emit('evento_realtime', { 
            type: 'NOVA_MENSAGEM', turmaId: req.params.turmaId, mensagem: novaMensagem, escolaId: 'DEFAULT'
        });
...
router.put('/perfil', verificarToken, async (req, res) => {
    try {
        ...
        if (bcrypt && user.senha && String(user.senha).startsWith('$2')) {
            ...
            const senhasValidas = [String(user.senha).trim(), String(user.senha_provisoria).trim(), String(user.senhaProvisoria).trim()];
...
router.put('/perfil/nome', verificarToken, async (req, res) => {
    try {
        ...
        await database.collection('workspace_entregas').updateMany({ alunoId: { $in: idsDoAluno } }, { $set: { alunoNome: nomeLimpo } });
        await database.collection('workspace_entregas_provas').updateMany({ alunoId: { $in: idsDoAluno } }, { $set: { alunoNome: nomeLimpo } });
...
// ============================================================================
// 💬 GESTÃO DE FEEDBACKS DO PROFESSOR E ENTREGAS
// ============================================================================
router.put('/entregas/:entregaId/feedback/:feedbackId', verificarToken, async (req, res) => {
    try {
        const database = await connectDB();
        await database.collection('workspace_entregas').updateOne({ id: req.params.entregaId, "feedbacks.id": req.params.feedbackId }, { $set: { "feedbacks.$.texto": req.body.texto } });
        if (global.workspaceStream) global.workspaceStream.emit('evento_realtime', { type: 'NOVO_FEEDBACK', entregaId: req.params.entregaId, escolaId: 'DEFAULT' });
        res.status(200).json({ success: true });
...
router.delete('/entregas/:entregaId/feedback/:feedbackId', verificarToken, async (req, res) => {
    try {
        const database = await connectDB();
        await database.collection('workspace_entregas').updateOne({ id: req.params.entregaId }, { $pull: { feedbacks: { id: req.params.feedbackId } } });
        if (global.workspaceStream) global.workspaceStream.emit('evento_realtime', { type: 'NOVO_FEEDBACK', entregaId: req.params.entregaId, escolaId: 'DEFAULT' });
        res.status(200).json({ success: true });
...
router.post('/entregas', verificarToken, async (req, res) => {
    try {
        ...
        try {
            ...
            const professoresEGestores = await database.collection('usuarios').find({ escolaId: escolaId, tipo: { $in: ['Professor', 'Gestor'] } }).toArray();
            ...
            if (professoresEGestores.length > 0) {
                ...
                if (notificacoesArray.length > 0) {
                    await database.collection('workspace_notificacoes').insertMany(notificacoesArray);
                    if (global.workspaceStream) global.workspaceStream.emit('evento_realtime', { type: 'NOVA_NOTIFICACAO', destinatarios: nomesDestinatarios, escolaId: escolaId });
                }
            }
        } catch (erroNoti) {}
...
router.post('/entregas/:id/feedback', verificarToken, async (req, res) => {
    try {
        ...
        await db.collection('workspace_entregas').updateOne({ id: entregaId }, { $push: { feedbacks: novoFeedback } });
        ...
        try {
            ...
            if (alunosAlvo.length > 0) {
                ...
                if (notificacoesArray.length > 0) {
                    ...
                    if (global.workspaceStream) global.workspaceStream.emit('evento_realtime', { type: 'NOVA_NOTIFICACAO', destinatarios: nomesDestinatarios, escolaId: escola });
                }
            }
...
// ============================================================================
// 🧹 ROTAS DE DESTRUIÇÃO E REATIVAÇÃO DO CHAT E ENTREGAS
// ============================================================================
