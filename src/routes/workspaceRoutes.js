...
// ============================================================================
// 🎭 ARENA: ESCOLHA DE PAPÉIS E DICAS INTELIGENTES DO MESTRE
// ============================================================================

// 1. Árbitro: Tranca a escolha do personagem e avisa o adversário
router.post('/arena/:salaId/escolher-papel', verificarToken, async (req, res) => {
    try {
        const { alunoId, papelEscolhido, papelRestante, cenarioTexto } = req.body;
        const salaId = req.params.salaId;

        // Emite um evento imediato para a sala avisando que o papel foi trancado
        if (global.workspaceStream) {
            global.workspaceStream.emit('evento_realtime', {
        ...
        res.json({ success: true, papel: papelEscolhido });
    } catch (error) {
        res.status(500).json({ error: 'Erro ao trancar a escolha do personagem.' });
    }
});

// 2. O Mestre Inteligente: Gera dicas com base no Cenário e no Papel
router.post('/arena/:salaId/ajuda', verificarToken, async (req, res) => {
    try {
        const { cenario, papel } = req.body;
        
        const Groq = require('groq-sdk');
        const chaveApi = process.env.GROQ_API_KEY;
        if (!chaveApi) return res.status(500).json({ error: 'Chave API em falta.' });
        
        const groq = new Groq({ apiKey: chaveApi.trim() });

        const systemPrompt = `Você é o "Mestre da Guilda", um assistente de ensino de Inglês.
        O aluno está participando de um Roleplay (Teatro) de Inglês.
        Cenário: "${cenario}"
        O papel do aluno é: "${papel}"
        
        Sua missão: Crie 3 frases curtas, naturais e muito úteis em INGLÊS que ESTE PERSONAGEM diria nesta exata situação.
        ...
        res.json({ success: true, sugestoes: resultado.sugestoes });
    } catch (error) {
        res.status(500).json({ error: 'O Mestre não conseguiu formular a dica.' });
    }
});
